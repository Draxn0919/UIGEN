...
test("renders chat interface with message list and input", () => {
  ...
  // With no messages, the empty state is shown instead of MessageList
...
test("scrolls when messages change", () => {
  ...
  // With no messages, empty state is shown and MessageList is not rendered
  expect(screen.queryByTestId("message-list")).toBeNull();

  // Update messages - this should render the ScrollArea + MessageList
  (useChat as any).mockReturnValue({
    ...mockUseChat,
    messages: [
      { id: "1", role: "user", content: "Hello" },
...
test("renders with correct layout classes", () => {
  // Provide messages so the ScrollArea renders
  (useChat as any).mockReturnValue({
    ...mockUseChat,
